...
void main() {
  patrolTest('FRB initialization completes successfully', ($) async {
  ...
  patrolTest('scanDevices returns list of DiscoveredDevice', ($) async {
    await RustLib.init();
    await initPanicHandler();
    await startMockMode();
    ...
    expect(
      devices.isNotEmpty,
      isTrue,
      reason: 'Mock mode should return at least one device',
    );
  ...
  patrolTest('mock mode streams heart rate data', ($) async {
    await RustLib.init();
    await initPanicHandler();
    await startMockMode();
    ...
    expect(
      updates.isNotEmpty,
      isTrue,
      reason: 'Should receive HR updates in mock mode',
    );
    ...
    if (updates.isNotEmpty) {
      ...
      expect(
        filteredBpm,
        greaterThan(0),
        reason: 'Filtered BPM should be positive',
      );
      expect(
        filteredBpm,
        lessThan(220),
        reason: 'Filtered BPM should be realistic',
      );
      ...
      if (rmssd != null) {
        expect(
          rmssd,
          greaterThan(0),
          reason: 'RMSSD should be positive if present',
        );
      }
      ...
      if (battery != null) {
        expect(
          ...
          reason: 'Battery should be >= 0',
        );
        expect(
          battery,
          lessThanOrEqualTo(100),
          reason: 'Battery should be <= 100',
        );
      }
    }
  ...
  patrolTest('connectDevice handles invalid device ID gracefully', ($) async {
    await RustLib.init();
    await initPanicHandler();
  ...
  patrolTest('stream subscription can be cancelled', ($) async {
    await RustLib.init();
    await initPanicHandler();
    await startMockMode();
  ...
  patrolTest('logging stream receives log messages', ($) async {
    await RustLib.init();
    await initPanicHandler();
    ...
    expect(
      logs.isNotEmpty,
      isTrue,
      reason: 'Should receive log messages from Rust',
    );
    ...
    if (logs.isNotEmpty) {
      ...
      expect(
        log.message,
        isNotEmpty,
        reason: 'Log message should not be empty',
      );
      expect(
        log.timestamp,
        greaterThan(BigInt.zero),
  ...
  patrolTest('multiple API calls in sequence work correctly', ($) async {
    await RustLib.init();
    await initPanicHandler();
  ...
  patrolTest('panic handler prevents app crashes', ($) async {
    await RustLib.init();
    await initPanicHandler();
